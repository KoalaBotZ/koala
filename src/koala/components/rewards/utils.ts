...
async function checkCooldown(user: User, type: string): Promise<{ canClaim: boolean; nextClaimTime: Date }> {
    ...
    const lastClaimed = (userData.cooldowns?.rewards?.[type as CooldownType] as Date) || new Date(0);
    const now = new Date();

    const cooldownDuration = getCooldownDuration(type);
    const nextClaimTime = new Date(lastClaimed.getTime() + cooldownDuration);

    return {
        canClaim: now.getTime() >= nextClaimTime.getTime(),
        nextClaimTime,
    };
}
...
async function updateUserRewards(user: User, type: string, pamonhas: number): Promise<void> {
    const updateData = {
        balance: (await db.users.get(user)).balance + pamonhas,
        [`cooldowns.rewards.${type}`]: new Date(),
    };
    await db.users.update(user, updateData);
}

export { checkCooldown, calculatePamonhas, updateUserRewards };