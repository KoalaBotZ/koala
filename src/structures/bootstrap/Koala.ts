...
export class Koala extends Client<true> {
    ...
    constructor(props: KoalaProps) {
        ...
        this.start().catch((error) => consola.error("Failed to start the bot:", error));
    }
    ...
    private async loadHandlers(): Promise<void> {
        if (this.koalaOptions?.loadEvents !== false) {
            await EventHandler.loadEvents(this);
        }
        if (this.koalaOptions?.loadCommands !== false) {
            await CommandHandler.loadCommands(this);
        }
        if (this.koalaOptions?.loadComponents !== false) {
